...
- v3.1.13（最新）：「范围」由原来单个输入框（要手填 `20` 或 `15-60`）拆成**起始 / 终止两个框**，填起来更直观；留空回退默认（起始 1、终止 20），填反了自动对调，旧配置里的单框值会自动拆到两个框
...
- 完整迭代历史：v1.1.0 → v3.1.13（当前发布版 v3.1.13，源码与 exe 都在仓库根目录）
