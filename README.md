...
- v3.1.11（最新）：新增「抓取原图」（默认开）——很多相册页只给缩略图地址（如 `0001_600x0.webp` = 600×800），程序现在会自动改试同目录原图（`0001.jpg` = 1800×2400），猜不到/取不到就自动回退预览图，不会因此失败；同时加了文件头校验（这类 CDN 对不存在的文件会返回 200+HTML，以前会把网页当图片存下来）
...
- 完整迭代历史：v1.1.0 → v3.1.11（当前发布版 v3.1.11，源码与 exe 都在仓库根目录）
